    ...
    try {
      ...
      const channelsList = channelsData.map((channel) => {
        const otherMembers = channel.members.filter(member => 
          member.user_id !== channel.createdBy
        );
        ...
        return {
          id: channel.id,
          cid: channel.cid,
          name: buyer?.user?.name || buyer?.user?.id || "Bilinmeyen Kullanıcı",
          buyerId: buyer?.user_id,
          id: buyer?.user_id || channel.id,
          lastMessage: channel.lastMessage?.text || "",
          time: channel.lastMessage?.created_at 
            ? new Date(channel.lastMessage.created_at).toLocaleTimeString("tr-TR", {
                hour: "2-digit",
                minute: "2-digit",
              })
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="p-3 md:p-4 border-b flex items-center justify-between bg-gray-50">
        <span className="text-base md:text-lg font-semibold text-gray-800">Müşteriler</span>
        <button
      ...
      {loading ? (
        ...
        <div className="flex-1 flex items-center justify-center text-gray-400">
          <div className="text-center p-8">
            ...
            <p className="text-sm">Müşteriler mesaj gönderdiğinde burada görünecek</p>
          </div>
        </div>
      ) : (
            ...
            >
              <div className="relative">
                <UserCircle2 
                  className={clsx(
                    "size-7 md:size-8",
                    selectedBuyer?.id === channel.id ? "text-gray-700" : "text-gray-400"
                  )} 
                />
